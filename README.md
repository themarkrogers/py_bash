...
(`.github/workflows/release-from-version.yml`) creates an annotated tag `vX.Y.Z` on that commit and pushes it if that
tag
does not already exist on the remote and no GitHub Release already exists for that same tag.
The same workflow then verifies tag/version consistency, builds wheel and sdist artifacts, and publishes a GitHub
